...
exports.decryt = (jweString, key) => {
  ...
  if (header.alg === 'RSA1_5')
  	cek = crypto.privateDecrypt({ key: key_to_use, padding: crypto.constants.RSA_PKCS1_PADDING }, encrypted_cek);
  else if (header.alg === 'RSA-OAEP')
  	cek = crypto.privateDecrypt({ key: key_to_use, padding: crypto.constants.RSA_PKCS1_OAEP_PADDING }, encrypted_cek);
