...
exports.decryt = (jweString, key) => {
  ...
  else if (header.alg === 'dir' || header.alg === 'A128KW' || header.alg === 'A256KW')
  	key_to_use = base64url.toBuffer(key);
  else
    throw new Error('unsupported algorithm detected in jwe header: ' + header.alg);
  ...
  else if (header.alg === 'A128KW')
    cek = keyUnWrap('aes-128-ecb', encrypted_cek, key);
  else if (header.alg === 'A256KW')
